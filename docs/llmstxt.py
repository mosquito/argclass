...
_PAGES: list[tuple[str, str, str, bool]] = [
    ("quickstart", "Quick Start", "5-minute overview of essential concepts", False),
    ("tutorial", "Tutorial", "30-minute guide building a complete CLI application", False),
    ("arguments", "Arguments", "Typed arguments, defaults, nargs, enums, boolean flags", False),
    ("groups", "Groups", "Reusable argument groups with prefix support", False),
    ("subparsers", "Subparsers", "Hierarchical subcommands and nested parsers", False),
    ("config-files", "Config Files", "INI, JSON, and TOML configuration file support", False),
    ("config-generation", "Generating Config Files", "Writing INI/JSON/TOML/.env from a parser", False),
    ("environment", "Environment Variables", "Automatic env var binding with prefixes", False),
    ("secrets", "Secrets", "Secret masking to prevent accidental exposure in logs", False),
    ("api", "API Reference", "Full API documentation with all classes and functions", False),
    ("examples", "Examples Gallery", "Ready-to-use patterns from simple CLIs to multi-command tools", True),
    ("errors", "Error Handling", "Error types and handling strategies", True),
    ("pitfalls", "Common Pitfalls", "Frequent mistakes and how to avoid them", True),
